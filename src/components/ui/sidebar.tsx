...
function SidebarMenuButton({
  ...
  // On mobile the sidebar is a sheet with every label already visible, so the tooltip
  // has nothing to add — and a mounted Radix tooltip consumes Escape before the sheet
  // can, leaving the drawer stuck open. Rendering it hidden is not enough; don't mount it.
  if (!tooltip || isMobile) {
    return button;
  }
...
function SidebarMenuSkeleton({
  ...
  const skeletonStyle = React.useMemo<SidebarStyle>(() => ({ "--skeleton-width": width }), [width]);
